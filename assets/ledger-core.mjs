...
const EXPENSE_CATEGORY_RULES = [
  ["餐饮", /餐厅|餐饮|早餐|午餐|晚餐|饭店|饭馆|面馆|面包|煎饼|小吃|烧烤|咖啡|外卖|美团外卖|饿了么|星巴克|瑞幸|肯德基|麦当劳|火锅|奶茶|茶饮|喜茶|奈雪/u],
...
export function inferCategory(description, direction = "expense") {
  return recommendCategory(description, direction).category;
}

export function recommendCategory(description, typeOrOptions = "expense") {
  const options =
    typeOrOptions && typeof typeOrOptions === "object"
      ? typeOrOptions
      : { type: typeOrOptions };
  const type = normalizeTransactionType(options.type || options.direction);
  ...
  if (historyRecommendation) {
    return {
      category: historyRecommendation.category,
      confidence: historyRecommendation.confidence,
      source: "user-history",
      merchant,
    };
  }

  const ruleCategory = matchRuleCategory(merchant || description, type);
  if (ruleCategory) {
    return {
...
export function buildMerchantCategoryHistory(transactions = []) {
  ...
  for (const transaction of transactions) {
    ...
    if (!isLearnableCategory(category, type)) {
      continue;
    }

    const merchant = extractMerchantKey(transaction.description);
    if (!merchant) {
      continue;
    }

    const key = getHistoryKey(merchant, type);
    const counts = history.get(key) || new Map();
...
function findHistoryRecommendation(history, merchant, type) {
  if (!history || !merchant || typeof history.get !== "function") {
    return null;
  }

  const counts = history.get(getHistoryKey(merchant, type));
  ...
  return {
    category: ranked[0][0],
    confidence: ranked[0][1] >= 2 ? "high" : "medium",
  };
}

function getHistoryKey(merchant, type) {
  return `${normalizeTransactionType(type)}|${merchant}`;
}
...
function isLearnableCategory(category, type) {
  ...
  return !isFallbackCategory(category, normalizedType) && CATEGORY_OPTIONS_BY_TYPE[normalizedType].includes(category);
}

function matchRuleCategory(description, type) {
...
export function isFallbackCategory(category, type = "expense") {
  ...
  return String(category || "").trim() === getFallbackCategory(normalizedType);
}

function getFallbackCategory(type) {
  return normalizeTransactionType(type) === "income" ? "其他收入" : "其他支出";
}
...
export function getTypeLabel(type) {
  const normalizedType = normalizeTransactionType(type);
  return TRANSACTION_TYPES.find((transactionType) => transactionType.value === normalizedType).label;
}
...
export function normalizeTransactionCategory(category, type = "expense", description = "") {
  const normalizedType = normalizeTransactionType(type);
  if (normalizedType === "transfer") {
    return "转账";
  }
    ...
    direction:
      type === "transfer"
        ? amount >= 0
          ...
          : "expense"
        : type,
    category: normalizeTransactionCategory(transaction.category, type, transaction.description),
  };

  if (type === "transfer") {
    next.type = "transfer";
    if (transaction.transferMatch === "explicit" || transaction.transferMatch === "auto") {
      next.transferMatch = transaction.transferMatch;
    } else {
  ...
  return normalizedTransactions.map((transaction, index) => {
    ...
    if (shouldBeTransfer) {
      return normalizeLedgerTransaction({
        ...transaction,
        type: "transfer",
        transferMatch: transaction.transferMatch === "explicit" ? "explicit" : "auto",
      });
    }
    if (transaction.type !== "transfer") {
      return normalizeLedgerTransaction(transaction);
    }
    if (transaction.transferMatch === "explicit") {
      return normalizeLedgerTransaction(transaction);
    }
